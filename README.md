...
基于图像分类的垃圾分类模型项目。当前数据集按类别编号组织在 `data/train/` 下，测试图片位于 `data/test/`。现有临时训练与推理脚本保留在 `temp/`，后续稳定版本可整理到 `src/`。
...
总体方案见 [docs/项目实施方案.md](docs/项目实施方案.md)。发布前检查见 [docs/GitHub发布检查.md](docs/GitHub发布检查.md)。
